...
function SalesPage() {
  ...
  const discountedSales = sales.filter(
    (product) =>
      product.discont_price > 0 && product.discont_price < product.price,
  );

  const filteredSales = discountedSales.filter((product) => {
  ...
  return (
    <Box sx={categoriesContainerStyle}>
      <Breadcrumbs separator="" aria-label="breadcrumb" sx={breadcrumbsStyle}>
        ...
        <NavLink to="/sales">
          <Button sx={activeBreadcrumbButtonStyle}>All sales</Button>
        </NavLink>
      </Breadcrumbs>
      <Box sx={categoriesContentStyle}>
        <Typography sx={categoriesTitleStyle}>Discounted items</Typography>
